...
export default function Study() {
  ...
  const handleReview = useCallback(async (correct: boolean) => {
    ...
    setIsPending(false);
  }, [currentWord, currentIndex, words, user]);
  ...
  return (
    <Layout>
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="relative h-80 cursor-pointer" style={{ perspective: "1000px" }} onClick={handleFlip}>
          <div
        ...
        {flipped && (
          <div className="flex gap-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
            <Button variant="outline" size="lg" className="flex-1 border-red-200 text-red-600 hover:bg-red-50" onClick={() => handleReview(false)} disabled={isPending}>
              <XCircle className="w-5 h-5 mr-2" /> Chưa nhớ
            </Button>
            <Button size="lg" className="flex-1 bg-green-600 hover:bg-green-700" onClick={() => handleReview(true)} disabled={isPending}>
