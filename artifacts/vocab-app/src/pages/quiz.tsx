...
return value.split(",").map((v) => normalizeAnswer(v)).filter(Boolean);
}
...
const allCorrect = correctWords.every((w) => selectedNormalized.includes(w)) &&
selectedNormalized.every((w) => correctWords.includes(w));
...
<p className="text-muted-foreground">Hãy thêm ít nhất 4 từ phù hợp bộ lọc đã chọn</p>
</div>
</Layout>
);
}
...
<p className="text-muted-foreground mt-2">Kiểm tra từ vựng theo nhiều chế độ</p>
</div>
...
<option value="fill-all">Điền tất cả đáp án</option>
<option value="multi-select">Chọn tất cả đáp án đúng</option>
<option value="mixed">Kết hợp</option>
</select>
<select className="w-full border rounded-lg p-2" value={questionCount} onChange={(e) => setQuestionCount(Number(e.target.value))}>
...
<select className="w-full border rounded-lg p-2" value={dateFilter} onChange={(e) => setDateFilter(e.target.value as DateFilter)}>
<option value="all">Tất cả từ</option>
<option value="today">Thêm hôm nay</option>
<option value="yesterday">Thêm hôm qua</option>
<option value="3days">Trong 3 ngày</option>
<option value="7days">Trong 7 ngày</option>
</select>
</div>
<Button size="lg" className="w-full max-w-xs" onClick={() => setStarted(true)}>Bắt đầu kiểm tra</Button>
</div>
</Layout>
);
}
...
<MascotCelebration message={celebMsg} subMessage={Bạn đạt ${score}/${questions.length}} />
<div className="text-6xl font-bold text-primary">{pct}%</div>
<div className="flex items-center gap-2 text-muted-foreground"><Clock className="w-4 h-4" />{formatTime(elapsedSeconds)}</div>
<div className="flex gap-2 flex-wrap justify-center">
{answers.map((correct, i) => (
<div key={i} className={w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold ${correct ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}}>{i + 1}</div>
))}
</div>
<div className="flex gap-3">
<Button variant="outline" onClick={handleRestart}><RefreshCw className="w-4 h-4 mr-2" />Làm lại</Button>
...
<p className="text-sm text-muted-foreground">Câu {currentIndex + 1} / {questions.length}</p>
</div>
<div className="text-right">
<div className="text-green-600 font-bold">{score}</div>
<div className="text-sm text-muted-foreground flex items-center gap-1"><Clock className="w-4 h-4" />{formatTime(elapsedSeconds)}</div>
</div>
</div>
...
<Progress value={progress} className="h-2" />  
    ...
    <div className="bg-card border rounded-2xl p-8">  
      {currentQ?.type === "multiple-choice" && (  
        <>  
          <p className="text-center text-sm text-muted-foreground mb-4">Từ này có nghĩa là gì?</p>  
          <h2 className="text-4xl font-bold text-center mb-6">{currentQ.term}</h2>  
          <div className="grid gap-3">  
            {currentQ.options.map((option, i) => {  
              const correct = option === currentQ.correctDefinition;  
              const isSelected = selected === option;  
              let cls = "w-full p-4 rounded-xl border-2 text-left ";  
              if (selected !== null) {  
                if (correct) cls += "bg-green-50 border-green-500";  
                else if (isSelected) cls += "bg-red-50 border-red-500";  
                else cls += "opacity-50";  
              }  
              return (  
      ...
      {currentQ?.type === "multi-select" && (  
        <>  
          <p className="text-center text-sm text-muted-foreground mb-4">Chọn tất cả từ đúng</p>  
          <h2 className="text-3xl font-bold text-center mb-6">{currentQ.correctDefinition}</h2>  
          <div className="grid grid-cols-2 gap-3">  
            {multiOptions.map((word) => {  
              const selectedNow = selectedWords.includes(word);  
              const isCorrect = currentQ.correctWords.includes(word);  
              let cls = "p-3 rounded-lg border ";  
              if (multiChecked) {  
                if (isCorrect && selectedNow) cls += "bg-green-100 border-green-500";  
                else if (isCorrect && !selectedNow) cls += "bg-green-50 border-green-200 opacity-80";  
                else if (!isCorrect && selectedNow) cls += "bg-red-100 border-red-500";  
                else cls += "opacity-40";  
              } else {  
                cls += selectedNow ? "border-primary bg-primary/5 text-primary font-medium" : "hover:bg-muted";  
              }  
              return (  
                <button key={word} className={cls} onClick={() => toggleMultiWord(word)}>  
                  {word}  
                </button>  
              );  
            })}  
          </div>  
          {!multiChecked && <Button className="w-full mt-4" onClick={handleMultiCheck}>Kiểm tra</Button>}  
        </>  
      )}  
    </div>  

    {((currentQ?.type === "multiple-choice" && selected !== null) ||  
      (currentQ?.type === "fill-all" && selected !== null) ||  
      (currentQ?.type === "multi-select" && multiChecked)) && (  
