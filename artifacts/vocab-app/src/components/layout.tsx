...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { href: "/words", label: "Quản lý từ vựng", icon: ListOrdered },
    { href: "/words/new", label: "Thêm từ mới", icon: PlusCircle },
  ];
      ...
      <aside className="w-64 bg-sidebar border-r border-sidebar-border flex flex-col shrink-0 fixed inset-y-0 left-0 z-20">
        ...
        <div className="px-5 py-5 border-b border-sidebar-border">
          <Link href="/" className="flex items-center gap-3">
            ...
            <span className="font-bold text-xl text-foreground tracking-tight">Lexify</span>
          </Link>
        </div>
